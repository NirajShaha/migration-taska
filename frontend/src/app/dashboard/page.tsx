...
import { Loader2, Search, ArrowRight, CheckCircle2, LogOut } from "lucide-react";
...
export default function Home() {
  ...
  // Client-side authentication check - redirect to login if not authenticated
  useEffect(() => {
  ...
  // If unauthenticated, don't render the dashboard content
  if (!session) {
    return null;
  }
  ...
  const handleSignOut = async () => {
    ...
    try {
      // Call backend logout endpoint if userId is available
      if (session?.user?.id) {
        try {
          const logoutUrl = `http://localhost:8081/auth/logout?userId=${session.user.id}`;
          ...
          // Continue with next-auth signout even if backend call fails
        }
      }
    ...
    try {
      ...
      if (response?.valid) {
        ...
        const mappedData = {
          ...
          generateTyreList:
            response.typGenTyrList || response.varGenTyrList || "",
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="mb-12 text-center">
            ...
            <p className="text-lg text-slate-300">
              Search and update vehicle variant information, certificates, and
